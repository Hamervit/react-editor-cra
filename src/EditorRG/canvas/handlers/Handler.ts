...
class Handler implements HandlerOptions {
    ...
    public set = (key: keyof FabricObject, value: any) => {
        ...
        const { id, superType, type, player, width, height } =
            activeObject as any;
        ...
        activeObject.clone((clonedObj: FabricObject) => {
            ...
            if (clonedObj.type === "activeSelection") {
                ...
                if (clonedObj.dblclick) {
                    clonedObj.on(
                        "mousedblclick",
                        this.eventHandler.object.mousedblclick as any
                    );
                }
                if (onAdd) {
                    onAdd(clonedObj);
            ...
            activeObject.clone((cloned: FabricObject) => {
                if (this.keyEvent.clipboard) {
                    if (cloned.superType === "node") {
                        ...
                        this.copyToClipboard(
                            JSON.stringify(
                                cloned.toObject(propertiesToInclude),
                                null,
                                "\t"
                            )
                        );
    ...
    public paste = () => {
        ...
        if (
            typeof clipboard.cloneable !== "undefined" &&
            !clipboard.cloneable
        ) {
            return false;
        }
    ...
    public setFabricObjects = (fabricObjects: FabricObjects) => {
        this.fabricObjects = Object.assign(
            {},
            this.fabricObjects,
            fabricObjects
        );
