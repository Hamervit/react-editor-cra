...
class CropHandler {
    ...
    public start = () => {
        if (this.validType()) {
            ...
            this.cropObject =
                this.handler.canvas.getActiveObject() as FabricImage;
    ...
    public resize = (opt: fabric.IEvent) => {
        ...
        if (corner === "tl") {
            if (Math.round(cropLeft) > Math.round(left)) {
                ...
                const targetRight = Math.round(
                    target.getBoundingRect().left +
                        target.getBoundingRect().width
                );
                const diffRightRatio =
                    1 - (originRight - targetRight) / cropWidth;
                target.set({
                    left: cropLeft,
            ...
            if (
                ...
                // right
                const diffLeftRatio =
                    1 - (original.left - cropLeft) / cropWidth;
                target.set({
                    left: original.left,
