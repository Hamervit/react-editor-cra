...
class EventHandler {
    ...
    public destroy = () => {
        if (this.handler.editable) {
            ...
            this.handler.getObjects().forEach((object) => {
                object.off(
                    "mousedown",
                    this.handler.eventHandler.object.mousedown as any
                );
                if (object.anime) {
                    anime.remove(object);
                }
            ...
            if (target.type === "activeSelection") {
                ...
                activeSelection.getObjects().forEach((obj: any) => {
                    ...
                    if (obj.superType === "node") {
                        ...
                        this.handler.elementHandler.setPositionByOrigin(
                            el,
                            obj,
                            left,
                            top
        ...
        if (editable) {
            ...
            if (this.handler.interactionMode === "polygon") {
                if (
                    ...
                    this.handler.drawingHandler.polygon.generate(
    ...
    public resize = (nextWidth: number, nextHeight: number) => {
        ...
        const diffScaleX =
            nextWidth /
            (this.handler.workarea.width * this.handler.workarea.scaleX);
        const diffScaleY =
            nextHeight /
                            ...
                            if (filteredObjects.length === 1) {
                                ...
                                objects.forEach((obj) => {
                                    ...
                                    if (obj.superType === "link") {
                                        obj.fromNodeId =
                                            nodes[obj.fromNodeIndex].id;
                                        obj.toNodeId =
                                            nodes[obj.toNodeIndex].id;
                                    } else {
                                        obj.left =
                                            obj.properties.left + padding;
                                        obj.top = obj.properties.top + padding;
                                    }
                                    const createdObj = this.handler.add(
                                        obj,
